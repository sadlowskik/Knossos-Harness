...
impl Agent {
    ...
    fn new_session(&self, params: &Value) -> Result<Value, RpcError> {
        ...
        let resumed_mission = params
            .get("resumeMission")
            .and_then(Value::as_str)
            .filter(|value| !value.trim().is_empty())
            .map(str::to_owned);
        if let Some(mission_id) = &resumed_mission {
            talos.restore_conversation(mission_id).map_err(|e| {
                RpcError::new(
                    INVALID_PARAMS,
                    format!("cannot restore mission {mission_id}: {e:#}"),
                )
            })?;
        }
        ...
        self.sessions.lock().expect("sessions").insert(
            ...
            Arc::new(Live {
                ...
                cancel,
                started: AtomicBool::new(resumed_mission.is_some()),
                cwd: root,
        ...
        Ok(json!({
            ...
            "resumedMission": resumed_mission,
        }))
    }
...
mod tests {
    ...
    impl Editor {
        fn connect(scripted: Vec<crate::engine::Response>) -> Self {
            Self::connect_with_recovery(scripted, None)
        }

        fn connect_with_recovery(
            scripted: Vec<crate::engine::Response>,
            recovery_identity: Option<&str>,
        ) -> Self {
            ...
            let recovery_identity = recovery_identity.map(str::to_owned);
            let build: Arc<BuildAgent> = Arc::new(move |root: &Path, cancel, approver| {
                ...
                if let Some(identity) = &recovery_identity {
                    t.enable_conversation_checkpoints(identity.clone())?;
                }
                Ok(t)
            });
    ...
    async fn a_prompt_runs_a_turn_and_reports_why_it_stopped() {
        ...
        assert!(reply["result"]["missionId"].is_string(), "{reply}");
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn a_new_editor_process_can_restore_and_continue_a_checkpointed_mission() {
        let (_dir, root) = workspace();
        std::fs::write(
        ...
        .expect("lockfile");
        let mission = {
            let mut editor = Editor::connect_with_recovery(
                vec![text_response("first answer"); 2],
                Some("same-policy"),
            );
            let session = editor.open(&root);
            let (_, reply) = editor.turn(&session, "inspect the workspace");
            reply["result"]["missionId"]
                .as_str()
                .expect("checkpointed mission id")
                .to_string()
        };
        let stored_environment = crate::mission::MissionStore::open(&root, &mission)
            .expect("stored mission")
            .state()
            .environment
            .clone()
            .expect("stored environment");
        let current_environment = crate::environment::EnvironmentFingerprint::discover(&root)
            .expect("current environment")
            .into_record();
        assert!(
            crate::environment::EnvironmentFingerprint::unchanged_since(&root, &stored_environment)
                .expect("environment comparison"),
        ...
        let opened = editor.call(
            "session/new",
            json!({"cwd": root, "mcpServers": [], "resumeMission": mission}),
        );
        assert_eq!(
            opened["result"]["resumedMission"],
            json!(mission),
            "{opened}"
        );
        ...
        let (_, reply) = editor.turn(&session, "continue from the checkpoint");
        assert!(reply["result"]["stopReason"].is_string(), "{reply}");
        assert_eq!(reply["result"]["missionId"], json!(mission), "{reply}");
    }
